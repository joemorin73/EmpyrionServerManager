    ...
    public class GameServerConnection : IGameServerConnection
    {
        ...
        public void Connect()
        {
            ...
            _client.OnConnected += async () =>
            {
                ...
                {
                    var newFormat = DateTime.Now.ToString("R") + " - " + "OnConnected Exception: {0}";
                    _traceSource.TraceEvent(TraceEventType.Error, 1, newFormat, ex.Message);
        ...
        {
            var newFormat = DateTime.Now.ToString("R") + " - " + format;
            _traceSource.TraceEvent(TraceEventType.Information, 2, newFormat, args);
        }

        public void DebugLog(String format, params object[] args)
        {
            var newFormat = DateTime.Now.ToString("R") + " - " + format;
            _traceSource.TraceEvent(TraceEventType.Verbose, 2, newFormat, args);
        }
        ...
        public Dictionary<int, Faction> GetFactions()
        {
            return _factionsById;
        }
        ...
        private void ProcessEvent_ChatMessage(Eleon.Modding.ChatInfo obj)
        {
            ...
            else
            {
                ...
                lock (_onlinePlayersInfoById)
                {
                    ...
                    _onlinePlayersInfoById.TryGetValue(obj.playerId, out player);
                }
